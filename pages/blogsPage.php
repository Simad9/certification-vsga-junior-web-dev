...
$query = "SELECT * FROM blogs ORDER BY `1d` DESC";
$result = mysqli_query($koneksi, $query);
$totalBlog = mysqli_num_rows($result);
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Blogs - DigiNova Tech</title>
</head>
  ...
  <main class="px-6 md:px-[120px] pt-[20px] pb-[60px] flex flex-col gap-[50px]">
    ...
    <section class="relative w-full overflow-hidden rounded-[35px] bg-gradient-to-b from-[#fff] to-[#ffd9a6] px-8 py-16 md:py-[70px] text-center">
      ...
      <div class="relative flex flex-col items-center gap-4">
        <span class="inline-flex items-center gap-2 rounded-full bg-white/80 px-5 py-2 font-manrope text-[13px] font-semibold tracking-wide text-biru ring-1 ring-oren/30 backdrop-blur">
          <span class="text-oren">//</span> Our Blog
        </span>
        <h1 class="font-manrope text-[34px] md:text-[44px] font-extrabold text-text">
          Blog <span class="bg-gradient-to-r from-oren to-biru bg-clip-text text-transparent">DigiNova Tech</span>
        </h1>
        <p class="font-sora text-[16px] md:text-[18px] text-abu max-w-[560px]">Beberapa tulisan yang dibuat oleh tim kami untuk kamu.</p>
      </div>
    </section>
    ...
    <section class="flex flex-col gap-6">
      <div class="flex items-center justify-between">
        <h2 class="font-manrope text-[24px] font-bold text-text">Semua Artikel</h2>
        <span class="font-sora text-[14px] text-abu"><?= $totalBlog ?> artikel</span>
      </div>
            ...
            <article class="group flex flex-col overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-black/5 transition hover:-translate-y-1.5 hover:shadow-xl hover:shadow-oren/10">
              ...
              <div class="flex flex-col gap-3 p-6">
                ...
                <div class="mt-auto flex flex-col gap-2 pt-2">
                  <a href="blogsDetail.php?id_blog=<?= $data['1d'] ?>" class="flex items-center justify-center gap-2 rounded-full bg-oren px-5 py-3 font-manrope text-[14px] font-semibold text-white transition hover:bg-orange-500 hover:shadow-lg hover:shadow-oren/30">Lihat Detail</a>
                  <?php if ($_SESSION['username'] == 'admin') : ?>
                    <a href="../php/hapus.php?id_blog=<?= $data['1d'] ?>" onclick="return confirm('Yakin ingin menghapus blog ini?')" class="flex items-center justify-center gap-2 rounded-full border-2 border-red-200 bg-white px-5 py-3 font-manrope text-[14px] font-semibold text-red-500 transition hover:border-red-500 hover:bg-red-500 hover:text-white">Hapus</a>
                  <?php endif; ?>
                </div>
              </div>
            </article>
      ...
      <?php else : ?>
        <div class="flex flex-col items-center gap-4 rounded-[30px] bg-white py-20 text-center shadow-sm ring-1 ring-black/5">
          <h3 class="font-manrope text-[24px] font-bold text-text">Belum ada artikel</h3>
          <p class="font-sora text-[14px] text-abu">Artikel akan muncul di sini setelah admin mempublikasikannya.</p>
        </div>
      <?php endif; ?>
    </section>
