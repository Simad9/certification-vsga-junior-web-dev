...
<head>
  ...
  <title>Dashboard Admin - DigiNova Tech</title>
</head>
...
<body class="bg-background1 font-sora text-text antialiased">
  <main class="px-6 md:px-[120px] pt-[20px] pb-[60px] flex flex-col gap-8">
    ...
    <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
      <div>
        <h1 class="font-manrope text-[30px] md:text-[36px] font-extrabold text-text">Dashboard Admin</h1>
        <p class="mt-1 font-sora text-[16px] text-abu">Halaman ini hanya untuk admin.</p>
      </div>
      <div class="flex flex-wrap gap-3">
        ...
        <a href="admin.php" class="rounded-full border-2 border-oren px-6 py-2.5 font-manrope text-[14px] font-semibold text-white bg-oren transition hover:bg-oren hover:text-white">Tambah Event</a>
        <a href="tambahBlog.php" class="rounded-full border-2 border-oren px-6 py-2.5 font-manrope text-[14px] font-semibold text-oren transition hover:bg-oren hover:text-white">Tambah Blog</a>
    ...
    <!-- Form -->
    <section class="w-fullrounded-[30px] bg-white p-8 shadow-sm ring-1 ring-black/5 md:p-10">
      <h2 class="font-manrope text-[22px] font-bold text-text">Upload Event Kita</h2>
      <p class="mt-1 font-sora text-[14px] text-abu">Lengkapi form berikut untuk mempublikasikan event baru.</p>

      <form action="../php/uploadFile.php" method="post" enctype="multipart/form-data" class="mt-6 flex flex-col gap-5">
        <div>
        ...
        <div>
          ...
          <input type="text" name="title" id="title" placeholder="Judul event" required class="mt-1.5 w-full rounded-xl border border-black/10 bg-background1/50 px-4 py-3 font-sora text-[14px] outline-none transition focus:border-oren focus:bg-white focus:ring-2 focus:ring-oren/20">
        </div>

        <div>
          <label for="deskripsi" class="font-manrope text-[14px] font-semibold text-text">Deskripsi</label>
          <textarea name="deskripsi" id="deskripsi" rows="6" placeholder="Tulis deskripsi event di sini..." required class="mt-1.5 w-full resize-none rounded-xl border border-black/10 bg-background1/50 px-4 py-3 font-sora text-[14px] outline-none transition focus:border-oren focus:bg-white focus:ring-2 focus:ring-oren/20"></textarea>
        </div>

        <button type="submit" name="submit" class="mt-2 w-full rounded-full bg-oren py-3.5 font-manrope text-[14px] font-semibold text-white transition hover:bg-orange-500 hover:shadow-lg hover:shadow-oren/30">Upload Event</button>
      </form>
    </section>
  </main>
