...
<head>
  ...
  <title>Log In - DigiNova Tech</title>
</head>
  ...
  <section class="flex min-h-screen flex-col items-center justify-center gap-10 px-6 py-10 lg:flex-row lg:gap-20 lg:px-[120px]">
    ...
    <div class="flex w-full max-w-[430px] flex-col items-center">
      <div class="w-full rounded-[30px] bg-white p-8 shadow-sm ring-1 ring-black/5 md:p-10">
        <h1 class="font-manrope text-[28px] font-extrabold text-text">Log In</h1>
        <p class="mt-1 font-sora text-[14px] text-abu">Selamat datang kembali di DigiNova Tech</p>

        <?php if ($error) : ?>
          <div class="mt-5 rounded-xl bg-red-50 px-4 py-3 font-sora text-[13px] font-semibold text-red-600 ring-1 ring-red-200">Username atau password salah.</div>
        <?php endif; ?>

        <form action="../php/login.php" method="post" class="mt-6 flex flex-col gap-4">
          <div>
          ...
          <button type="submit" name="submit" class="mt-2 w-full rounded-full bg-oren py-3.5 font-manrope text-[14px] font-semibold text-white transition hover:bg-orange-500 hover:shadow-lg hover:shadow-oren/30">Log In</button>
        </form>
        ...
        <a href="signupPage.php" class="block w-full rounded-full border-2 border-oren py-3.5 text-center font-manrope text-[14px] font-semibold text-oren transition hover:bg-oren hover:text-white">Daftar Akun Baru</a>

        <!-- NOTE CREDENTIAL DEMO -->
        <div class="mt-6 rounded-2xl bg-background1 p-4 ring-1 ring-oren/25">
          <p class="font-manrope text-[13px] font-bold text-text">Kredensial Demo:</p>
          <div class="mt-2 flex flex-col gap-1.5 font-sora text-[13px] text-abu">
            <p>Admin : <span class="font-semibold text-text">admin</span> / <span class="font-semibold text-text">admin123</span></p>
            <p>User&nbsp;&nbsp;: <span class="font-semibold text-text">user</span> / <span class="font-semibold text-text">user123</span></p>
          </div>
        </div>
      </div>
